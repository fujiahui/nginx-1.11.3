...
ngx_slab_init(ngx_slab_pool_t *pool)
{
	...
	// slot 首先是一个完整的数组，当被使用之后才是就用链表形式管理起来
    for (i = 0; i < n; i++) {
        slots[i].slab = 0;
	...
	// 计算一共能够保存多少个页，加上ngx_slab_page_t,是因为每一个页都会有一个ngx_slab_page_t来
...
ngx_slab_alloc_locked(ngx_slab_pool_t *pool, size_t size)
{
    ...
    if (page->next != page) {
        ...
        if (shift < ngx_slab_exact_shift) {
            ...
            do {
				...
				// 计算shift对应的bitmap使用chunk个数
                map = (1 << (ngx_pagesize_shift - shift))
                          / (sizeof(uintptr_t) * 8);

                for (n = 0; n < map; n++) {

					// 第n个bitmap是否已经使用完，查找未使用完的chunk
					// 此处其实可以先跳过bitmap使用的chunk开始遍历
                    if (bitmap[n] != NGX_SLAB_BUSY) {

                        for (m = 1, i = 0; m; m <<= 1, i++) {
							// 如果第n个bitmap的第m位已经使用，则继续寻找下一个
                            if ((bitmap[n] & m)) {
                                continue;
                            }
            ...
            do {
				// 如果不等于mask说明还有空闲chunk可以使用
                if ((page->slab & NGX_SLAB_MAP_MASK) != mask) {

                    for (m = (uintptr_t) 1 << NGX_SLAB_MAP_SHIFT, i = 0;
                         m & mask;	// 如果越过了mask区域就结束
                         m <<= 1, i++)
                    {
                    	// 如果已经被使用就寻找下一个chunk
                        if ((page->slab & m)) {
    ...
    if (page) {
        if (shift < ngx_slab_exact_shift) {
			...
			/* 一个chunk为s字节大小
			 * 一共有(1 << (ngx_pagesize_shift - shift)) 个chunk数
			 * 一个字节可以标记8个chunk的分配情况
			 * n: 需要n个chunk用来作为bitmap使用
			**/
            s = 1 << shift;	// 一个chunk为s字节大小
			...
			/* 前n个chunk用来作为bitmap使用 
			 * 随后一个为最新申请的chunk
			**/
            bitmap[0] = (2 << n) - 1;

			/*一个uintptr_t可用来标识32个chunk，sizeof(uintptr_t) * 8=32*/
			...
			/* 申请的chunk位置首先得跳过bitmap使用的n个chunk*/
